<script lang="ts">
	...
	let formData = $state<Partial<CreateArtist | UpdateArtist>>({
		...
		worker_category: artist?.worker_category || '',
		position_status: artist?.position_status || undefined,
		hire_date: artist?.hire_date || '',
		termination_date: artist?.termination_date || '',
		llc_name: artist?.llc_name || '',
		onboarding_complete: artist?.onboarding_complete ?? true,
		// Initialize arrays
