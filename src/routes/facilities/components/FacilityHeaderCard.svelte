	...
	<div class="card-body p-4">
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
			...
			<div class="lg:col-span-2 space-y-3">
				...
				<div class="space-y-3">
					...
					<div class="flex items-center gap-3 flex-wrap">
						...
						<InlineEditableField
							...
							maxLength={200}
							onSave={(value) => onUpdateField('reference', value)}
				...
				<div class="flex items-center gap-3 pt-2 border-t border-base-300">
					<div class="stat stat-compact p-0">
						...
						<div class="stat-value text-lg">{locationCount}</div>
					</div>
					{#if upcomingEventsCount > 0}
						<div class="stat stat-compact p-0">
							<div class="stat-title text-xs">Upcoming Events</div>
							<div class="stat-value text-lg">{upcomingEventsCount}</div>
						</div>
					{/if}
					<div class="stat stat-compact p-0">
						<div class="stat-title text-xs">Created</div>
						<div class="stat-value text-sm">{formatDate(facility.created_at)}</div>
					</div>
				</div>
			</div>
			...
			<div class="lg:col-span-1">
				<div class="space-y-2">
					<h3 class="text-sm font-semibold opacity-70">Featured Image</h3>
					<div class="space-y-2">
						<InlineEditableField
