	...
	async function saveEdit() {
		...
		try {
			...
			if (false) { // basic tab removed
				...
				const existingSetlistNotes = typeof (event as any).setlist_review_notes === 'string' ? (event as any).setlist_review_notes : ''
				if (formData.setlist_review_notes !== existingSetlistNotes) {
				...
				{#if editingTab === 'notes'}
					<div class="space-y-4">
						...
						></textarea>
						 ...
						 <div class="form-control">
						 <label class="label" for="setlist-review-notes-edit">
						 <span class="label-text">Setlist review notes</span>
						</label>
						<textarea
						 id="setlist-review-notes-edit"
						  bind:value={formData.setlist_review_notes}
						 class="textarea textarea-bordered"
						  placeholder="Add setlist review notes (optional)..."
						  rows="4"
						></textarea>
						</div>
						</div>
					...
					<div>
						<InlineEditableField
							...
							formatDisplay={(val) => val !== null && val !== undefined && val !== '' ? String(val) : 'Not specified'}
						/>
					</div>
				</div>
			{/if}
