	...
	async function saveEdit() {
		...
		try {
			...
			if (false) { // basic tab removed
				...
				// Always include setlist_review_notes to allow saving or clearing
				updateData.setlist_review_notes = formData.setlist_review_notes || null
			}
			...
			<div class="space-y-4">
				...
				{:else}
					<div class="space-y-4">
						...
						{#if event.setlist_review_notes}
							<p class="text-base whitespace-pre-wrap">{event.setlist_review_notes}</p>
						{:else}
							<p class="text-base opacity-50">No setlist review notes</p>
						{/if}
					</div>
					</div>
				{/if}
