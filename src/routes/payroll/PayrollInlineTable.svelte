				...
				<tbody>
					{#each displayEntries as entry (entry.id)}
						...
						<tr class:bg-warning={isNew} class:bg-opacity-20={isNew}>
							...
							<!-- Source -->
							<td>
								{#if entry.creation_method === 'event-automation'}
									<div class="tooltip" data-tip={`From event ${entry.source_event_id || ''}`}>
								...
								{:else if entry.creation_method === 'manual'}
									<div class="tooltip" data-tip={`Created by ${entry.created_by || 'Unknown'}`}>
										<span class="badge badge-outline badge-xs">Manual</span>
									</div>
								{:else}
