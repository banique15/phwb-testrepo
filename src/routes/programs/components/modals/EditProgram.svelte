<script lang="ts">
	...
	// Form state
	let formData = $state<UpdateProgram>({})
	...
	$effect(() => {
		if (program && open) {
			formData = {
				...
				geo_coverage: program.geo_coverage || '',
				start_date: program.start_date || '',
				end_date: program.end_date || '',
				partner: program.partner || undefined
			}
		}
	})

	// Validation helper
	function validateField(field: keyof UpdateProgram, value: any) {
		try {
			const fieldSchema = updateProgramSchema.shape[field]
			if (fieldSchema) {
	...
	// Form handlers
	function handleInputChange(field: keyof UpdateProgram, value: any) {
	...
	async function handleSubmit(event?: SubmitEvent | MouseEvent) {
		...
		try {
			updateProgramSchema.parse(formData)
		} catch (error) {
			if (error instanceof z.ZodError) {
		...
		try {
			...
			Object.keys(cleanData).forEach(key => {
				const value = cleanData[key as keyof UpdateProgram]
			...
			dispatch('success', { program: updatedProgram })
			dispatch('close')
		} catch (error) {
			console.error('Failed to update program:', error)
			submitError = error instanceof Error ? error.message : 'Failed to update program. Please try again.'
		} finally {
			isLoading = false
		}
	}
...
>
	{#if program}
		<form onsubmit={handleSubmit} class="space-y-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				<div class="space-y-4">
					...
					<div class="form-control">
						<label class="label" for="edit-program-title">
					...
					<div class="form-control">
						<label class="label" for="edit-program-geo-coverage">
							<span class="label-text">Geographic Coverage</span>
						</label>
						<input 
							id="edit-program-geo-coverage"
							type="text" 
				...
				<div class="space-y-4">
					...
					<div class="form-control">
						<label class="label" for="edit-program-start-date">
					...
					<div class="form-control">
						<label class="label" for="edit-program-end-date">
			...
			<div class="space-y-4">
				...
				<div class="form-control">
					<label class="label" for="edit-program-description">
	...
	{:else}
		<div class="text-center py-8">
			...
			<p class="text-sm opacity-60">Please select a program to edit</p>
		</div>
	{/if}
	...
	<svelte:fragment slot="actions" let:closeModal>
		...
		<button 
			...
			disabled={isLoading || !formData.title?.trim() || !program}
		>
			{#if isLoading}
